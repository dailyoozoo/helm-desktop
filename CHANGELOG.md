...
- 一键更新：内置官方发布源，设置里没有自定义更新源时「检查更新」可直接下载并安装新版本（此前只能打开下载页手动装）。发布侧新增 `release:latest-json` 脚本生成更新描述文件。

### Fixed

- 引擎安装后「先报错、再成功」的观感问题：① 安装命令结束到复检完成之间，就绪卡片保持「安装完成，正在复检…」，不再瞬间闪回「未安装」；② 就绪检查行在「未安装」时使用中性圆点而不是红色叉号（未安装是待办，不是故障），琥珀/红色只留给真正的失败；③ 未就绪时只打开就绪检查弹层，不再额外弹出红色提示。
- 「安装命令已成功，但复检未找到 CLI」的误报：安装成功后复检改为最多 3 次（间隔 1 秒、2 秒），覆盖 shim 刚落盘被杀软扫描暂不可见、以及 CLI 首次 `--version` 较慢的情况（实测 Codex 首次启动 11 秒）；单次检测超时上限由 10 秒放宽到 25 秒。
- Windows 中文环境下引擎检测与安装提示的乱码：`cmd /C` 包装后的子进程输出按系统 OEM 代码页解码，不再显示为不可读字符。

### Changed

- 界面关键时序写入日志：提示条、就绪项状态、安装向导探测结果会记录到应用日志，报「先报错后成功」这类顺序问题时可直接从日志还原，不依赖截图。

## [0.6.3] - 2026-09-25

### Fixed

- AI 配置页空状态对齐原型：执行引擎「未绑定服务商」提示条的图标被撑满整行，收为 16px 细提示条；服务商为空时改为虚线引导区并保留「添加服务商」按钮（此时它是唯一添加入口）；模型空目录字号从继承的 14px 对齐为 11.5px 居中虚线框。
